...
export default function AddProductPage() {
  ...
  const getInitialFormData = (): ProductForm => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('draft-product-form')
      if (saved) {
        try {
  ...
  const [formData, setFormData] = useState<ProductForm>(getInitialFormData)

  // Load saved images from localStorage
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedImages = localStorage.getItem('draft-product-images')
      if (savedImages) {
        try {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('draft-product-form', JSON.stringify(formData))
    }
  }, [formData])
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('draft-product-images', JSON.stringify(productImages))
    }
  }, [productImages])
  ...
  const validateForm = (): boolean => {
    ...
    // Georgian description is optional
  ...
  const clearDraft = () => {
    if (typeof window !== 'undefined') {
      localStorage.removeItem('draft-product-form')
      localStorage.removeItem('draft-product-images')
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(locale === 'ka' 
        ? `პროდუქტის შექმნა ვერ მოხერხდა: ${errorMessage}` 
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-gray-600">
                {locale === 'ka' 
                  ? 'შეავსეთ ყველა ველი ახალი პროდუქტის შესაქმნელად' 
            ...
            >
              {locale === 'ka' ? 'ფორმის გასუფთავება' : 'Clear Form'}
            </button>
          </div>
        </motion.div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    <span>{locale === 'ka' ? 'პროდუქტის შექმნა' : 'Create Product'}</span>
                  </>
                )}
              </button>
            </div>
